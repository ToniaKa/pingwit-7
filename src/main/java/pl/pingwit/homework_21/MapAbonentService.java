...
public class MapAbonentService {
    ...
    public Map<String, List<Abonent>> groupByCity(List<Abonent> abonents) { // постфикс Service в названии метода не нужен
        Map<String, List<Abonent>> abonentByCity = abonents.stream()
                .collect(Collectors.groupingBy(Abonent::getCity));
        return abonentByCity;
    }

    public Map<String, Abonent> mapNonVipByIds(List<Abonent> abonents) { // метод мапит не-вип контракты по id. mapNonVipByIds как вариант имени
        Map<String, Abonent> abonentsContracts = abonents.stream()
                .filter(abonent -> !abonent.getVip()) // условие можно заменить на !abonent.getVip()
