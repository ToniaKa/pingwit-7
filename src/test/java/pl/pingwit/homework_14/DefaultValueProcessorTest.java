...
class DefaultValueProcessorTest {
    ...
    void shouldSetDefaultValue_whenStringNull() throws IllegalAccessException {
        ...
        // достаточно expected
        CustomerDetails expected = new CustomerDetails("defaultValue", "defaultValue", "defaultValue", "defaultValue", LocalDate.now(), 7);

        defaultValueProcessor.process(customerDetailsWithStringNull);
        assertThat(customerDetailsWithStringNull).isEqualTo(expected);
    }
    ...
    void shouldSet0_whenIntegerNull() throws IllegalAccessException {
        ...
        CustomerDetails expected = new CustomerDetails("test", "test", "test", "test", LocalDate.now(), 0);

        defaultValueProcessor.process(customerDetailsWithIntegerNull);
        assertThat(customerDetailsWithIntegerNull).isEqualTo(expected);
    }
